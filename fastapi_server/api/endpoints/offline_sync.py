...
class OfflineEventRequest(BaseModel):
    ...
    priority: int = Field(
        default=1, ge=1, le=3, description="優先度 (1=高, 2=中, 3=低)"
    )
    force_queue: bool = Field(
        default=False, description="オンライン時でも強制的にキューに追加"
    )
...
async def queue_events_for_offline(request: OfflineEventRequest):
    ...
    if len(request.events) > 100:
        raise HTTPException(
            status_code=413, detail="Too many events. Maximum 100 events per request."
        )
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to queue events: {str(e)}")
...
async def sync_offline_events_endpoint(
    ...
    try:
        ...
        if queue_status["sync_in_progress"] and not request.force_sync:
            return {"message": "Sync already in progress", "queue_status": queue_status}

        if queue_status["total_queued_events"] == 0:
            return {"message": "No events to sync", "queue_status": queue_status}
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to sync offline events: {str(e)}"
        )
...
async def get_offline_sync_status():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get offline sync status: {str(e)}"
        )
...
async def clear_offline_queue():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to clear offline queue: {str(e)}"
        )
...
async def get_failed_events():
    ...
    try:
            ...
            failed_details.append(
                {
                    ...
                    "last_retry_at": (
                        event.last_retry_at.isoformat() if event.last_retry_at else None
                    ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get failed events: {str(e)}"
        )
...
def _generate_recommendations(queue_status: Dict[str, Any]) -> List[str]:
    ...
    if capacity_used > 0.8:
        recommendations.append(
            "Queue capacity is high. Consider increasing sync frequency."
        )

    if failed_events > 20:
        recommendations.append(
            "Many failed events detected. Review error logs and server status."
        )
