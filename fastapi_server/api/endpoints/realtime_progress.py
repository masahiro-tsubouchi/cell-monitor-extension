...
async def get_student_progress(user_id: str):
    ...
    try:
        ...
        if not progress:
            raise HTTPException(
                status_code=404, detail=f"Progress data not found for user: {user_id}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get student progress: {str(e)}"
        )
...
async def get_assignment_progress(user_id: str, assignment_id: str):
    ...
    """
    try:
        progress = realtime_progress_manager.get_assignment_progress(
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get assignment progress: {str(e)}"
        )
...
async def get_all_students_progress(
    ...
    try:
        ...
        stats = {
            ...
            "active_students": sum(
                1
                for p in limited_progress
                if p.last_activity_at
                and (datetime.now() - p.last_activity_at).total_seconds() < 3600
            ),
            "average_progress": (
                sum(p.overall_progress_percentage for p in limited_progress)
                / len(limited_progress)
                if limited_progress
                else 0
            ),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get all students progress: {str(e)}"
        )
...
async def get_user_notifications(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get user notifications: {str(e)}"
        )
...
async def get_progress_stats():
    ...
    try:
        ...
        enhanced_stats = {
            **stats,
            "total_students": len(all_progress),
            "total_assignments": sum(len(p.assignments_progress) for p in all_progress),
            "completion_rate": (
                sum(p.overall_progress_percentage for p in all_progress)
                / len(all_progress)
                if all_progress
                else 0
            ),
            "system_health": (
                "healthy" if stats.get("total_events_processed", 0) > 0 else "idle"
            ),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get progress stats: {str(e)}"
        )


@router.post("/process-event", status_code=202)
async def process_progress_event(
    user_id: str, assignment_id: str, cell_id: str, event_data: Dict[str, Any]
):
    """
    進捗イベントを処理する
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to process progress event: {str(e)}"
        )
    ...
    try:
        ...
        if initial_progress:
            await websocket.send_json(
                {"type": "initial_progress", "data": initial_progress.model_dump()}
            )
        ...
        while True:
            ...
            try:
                ...
                if message.get("type") == "ping":
                    ...
                    current_progress = realtime_progress_manager.get_student_progress(
                        user_id
                    )
                    if current_progress:
                        await websocket.send_json(
        ...
        while True:
            try:
                ...
                if message.get("type") == "ping":
                    ...
                    current_progress = (
                        realtime_progress_manager.get_all_students_progress()
                    )
                    ...
                    await websocket.send_json(
                        {
                            ...
                            "data": {
                                "students_progress": [
                                    p.model_dump() for p in current_progress
                                ],
                                "stats": current_stats,
                            },
                        }
                    ...
                    if student_id:
                        student_progress = (
                            realtime_progress_manager.get_student_progress(student_id)
                        )
                        if student_progress:
                            await websocket.send_json(
                                {
...
async def simulate_progress_event(
    ...
    try:
        ...
        test_event_data = {
            ...
            "errorMessage": (
                "Test error message" if event_type == "cell_execution_error" else None
            ),
            "code": "print('Hello, World!')",
            "result": (
                "Hello, World!" if event_type == "cell_execution_complete" else None
            ),
            "metadata": {"memory_usage_mb": 64.5, "cpu_usage_percent": 12.3},
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to simulate progress event: {str(e)}"
        )
...
async def reset_user_progress(user_id: str):
    ...
    try:
        ...
        keys_to_delete = [
            key
            for key in realtime_progress_manager.cell_progress.keys()
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to reset user progress: {str(e)}"
        )
