...
async def get_current_environment():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to collect current environment: {str(e)}"
        )
...
async def get_environment_summary_endpoint():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get environment summary: {str(e)}"
        )
...
async def create_environment_snapshot(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create environment snapshot: {str(e)}"
        )


@router.get("/packages", status_code=200)
async def get_package_information(
    package_names: Optional[List[str]] = Query(None, description="取得するパッケージ名のリスト"),
    ...
    try:
        ...
        if package_names:
            ...
            filtered_packages = [
                ...
                if pkg.name.lower() in package_names_lower
            ]
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get package information: {str(e)}"
        )
...
async def check_environment_health():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to check environment health: {str(e)}"
        )


@router.get("/diff", status_code=200)
async def get_environment_diff(
    from_snapshot_id: Optional[str] = Query(None, description="比較元スナップショットID"),
    ...
    try:
        ...
        if environment_collector._last_snapshot and environment_collector._last_snapshot.snapshot_id != current_snapshot.snapshot_id:
            ...
            return {
                ...
                "changes_detected": (
                    len(diff.added_packages) > 0 or
                    len(diff.removed_packages) > 0 or
                    len(diff.updated_packages) > 0
                )
            }
        else:
            return {
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get environment diff: {str(e)}"
        )
...
async def analyze_environment():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to analyze environment: {str(e)}"
        )


def _calculate_environment_health_score(snapshot: ExecutionEnvironmentSnapshot) -> float:
    """環境健全性スコアを計算する"""
    score = 1.0
...
def _generate_environment_recommendations(snapshot: ExecutionEnvironmentSnapshot) -> List[str]:
    """環境改善の推奨事項を生成する"""
    recommendations = []
    ...
    if major < 3 or (major == 3 and minor < 9):
        recommendations.append(f"Consider upgrading Python from {python_version} to 3.10+ for better performance and security")
    
    # メモリ使用量の推奨
    if snapshot.memory_usage_mb and snapshot.memory_usage_mb > 2000:  # 2GB以上
        recommendations.append("High memory usage detected. Consider optimizing data structures or using memory-efficient libraries")
    
    # 仮想環境の推奨
    if not snapshot.python_env.virtual_env and not snapshot.python_env.conda_env:
        recommendations.append("Consider using a virtual environment (venv or conda) for better dependency management")
    
    # パッケージ管理の推奨
    if snapshot.python_env.total_packages_count > 300:
        recommendations.append("Large number of packages detected. Consider creating separate environments for different projects")
...
def _detect_environment_warnings(snapshot: ExecutionEnvironmentSnapshot) -> List[str]:
    ...
    if 'numpy' not in package_names:
        warnings.append("NumPy not detected - many data science operations may not work")
    
    if 'pandas' not in package_names and 'numpy' in package_names:
        warnings.append("Pandas not detected - data manipulation capabilities may be limited")
    
    # システムリソースの警告
    if snapshot.system_env.memory_total_gb and snapshot.system_env.memory_total_gb < 4:
        warnings.append("Low system memory detected (< 4GB) - performance may be limited")
...
def _perform_environment_analysis(snapshot: ExecutionEnvironmentSnapshot) -> Dict[str, Any]:
    """環境情報の詳細分析を実行する"""
    
    # パフォーマンス指標
    performance_indicators = {
        "python_version_score": _score_python_version(snapshot.python_env.python_version),
...
def _check_python_compatibility(snapshot: ExecutionEnvironmentSnapshot) -> Dict[str, Any]:
    """Python互換性をチェックする"""
    return {
...
def _detect_package_conflicts(snapshot: ExecutionEnvironmentSnapshot) -> List[str]:
    ...
    if 'tensorflow' in package_names and 'torch' in package_names:
        conflicts.append("TensorFlow and PyTorch both installed - may cause memory issues")
    
    return conflicts


def _check_jupyter_compatibility(snapshot: ExecutionEnvironmentSnapshot) -> Dict[str, Any]:
    """Jupyter互換性をチェックする"""
    return {
