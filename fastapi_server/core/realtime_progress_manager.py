...
class RealtimeProgressManager:
    ...
    def __init__(self):
        ...
        self.assignment_progress: Dict[str, Dict[str, AssignmentProgressInfo]] = (
            defaultdict(dict)
        )
        self.cell_progress: Dict[str, Dict[str, CellProgressInfo]] = defaultdict(dict)

        # 通知管理
        self.pending_notifications: Dict[str, List[ProgressNotification]] = defaultdict(
            list
        )
        self.notification_history: deque = deque(maxlen=1000)

        # WebSocket接続管理
        self.active_subscribers: Dict[str, Set[str]] = defaultdict(
            set
    ...
    async def process_cell_execution_event(
        self, user_id: str, assignment_id: str, cell_id: str, event_data: Dict[str, Any]
    ) -> None:
        """
    ...
    async def _update_cell_progress(
        ...
        if event_data.get("eventType") == "cell_execution_start":
            ...
            cell_progress.execution_duration_ms = event_data.get(
        ...
        if cells:
            ...
            if assignment_progress.total_cells > 0:
                assignment_progress.progress_percentage = (
                    assignment_progress.completed_cells
                    / assignment_progress.total_cells
                    * 100
                )
            ...
            # 開始時刻を設定（初回のみ）
            if (
                not assignment_progress.started_at
                and assignment_progress.completed_cells > 0
            ):
                assignment_progress.started_at = datetime.now()

            # 完了予想時刻を計算
            if assignment_progress.overall_status == ProgressStatus.IN_PROGRESS:
                assignment_progress.estimated_completion_time = (
                    self._estimate_completion_time(assignment_progress)
                )
    ...
    async def _update_student_progress(self, user_id: str) -> StudentProgressSummary:
        ...
        if assignments:
            ...
            student_progress.total_execution_time_ms = sum(
                assignment.total_execution_time_ms
                for assignment in assignments.values()
            )

            if student_progress.total_execution_count > 0:
                student_progress.average_execution_time_ms = (
                    student_progress.total_execution_time_ms
                    / student_progress.total_execution_count
                )

            # エラー率を計算
            total_cells = sum(
                assignment.total_cells for assignment in assignments.values()
            )
            total_error_cells = sum(
                assignment.error_cells for assignment in assignments.values()
            )
            if total_cells > 0:
            ...
            # 初回活動時刻を設定（初回のみ）
            if (
                not student_progress.first_activity_at
                and student_progress.total_execution_count > 0
            ):
    ...
    async def _calculate_learning_metrics(
        self, student_progress: StudentProgressSummary
    ) -> None:
        """学習分析指標を計算する"""
        ...
        try:
            ...
            if student_progress.first_activity_at and student_progress.last_activity_at:
                time_diff = (
                    student_progress.last_activity_at
                    - student_progress.first_activity_at
                )
                hours = time_diff.total_seconds() / 3600
                if hours > 0:
                    student_progress.learning_velocity = (
                        student_progress.completed_assignments / hours
                    )
            ...
            if student_progress.total_assignments > 0:
                completion_rate = (
                    student_progress.completed_assignments
                    / student_progress.total_assignments
                )
                student_progress.consistency_score = completion_rate

        except Exception as e:
            logger.warning(
                f"Failed to calculate learning metrics for {student_progress.user_id}: {e}"
            )

    def _estimate_completion_time(
        self, assignment_progress: AssignmentProgressInfo
    ) -> Optional[datetime]:
        """課題完了予想時刻を計算する"""
        ...
        try:
            ...
            completed_cells = [
                cell
                for cell in assignment_progress.cells_progress
            ...
            # 残りセル数
            remaining_cells = (
                assignment_progress.total_cells - assignment_progress.completed_cells
            )

            # 予想残り時間（バッファを含む）
            estimated_remaining_ms = (
                remaining_cells * avg_duration_ms * 1.5
            )  # 50%バッファ
    ...
    async def _broadcast_progress_update(
        self, update_event: ProgressUpdateEvent
    ) -> None:
        """進捗更新をWebSocketで配信する"""
        ...
        try:
            ...
            for connection_id in user_connections:
                await manager.send_personal_message(
                    json.dumps(
                        {"type": "progress_update", "data": update_event.model_dump()}
                    ),
                    connection_id,
                )
    ...
    async def _generate_progress_notifications(
        ...
        try:
            ...
            for milestone in milestones:
                if progress_percentage >= milestone and not self._milestone_notified(
                    user_id, assignment_id, milestone
                ):
            ...
            for notification in notifications:
                self.pending_notifications[notification.recipient_id].append(
                    notification
                )
    ...
    def _milestone_notified(
        self, user_id: str, assignment_id: str, milestone: int
    ) -> bool:
        """マイルストーン通知済みかチェックする（簡易実装）"""
        # 実際の実装では永続化ストレージを使用
        key = f"{user_id}:{assignment_id}:{milestone}"
        return hasattr(self, "_milestone_flags") and key in getattr(
            self, "_milestone_flags", set()
        )

    def _set_milestone_notified(
        self, user_id: str, assignment_id: str, milestone: int
    ) -> None:
    ...
    async def _send_notification(self, notification: ProgressNotification) -> None:
        ...
        try:
            ...
            if notification.recipient_type == "student":
                user_connections = self.active_subscribers.get(
                    notification.recipient_id, set()
                )
                for connection_id in user_connections:
            ...
            await redis_client.publish(
                "progress_notifications", json.dumps(notification.model_dump())
            )

        except Exception as e:
            logger.error(
                f"Failed to send notification {notification.notification_id}: {e}"
            )

    async def _handle_processing_error(
        self, user_id: str, assignment_id: str, cell_id: str, error: Exception
    ) -> None:
        """処理エラーをハンドリングする"""
    ...
    async def subscribe_user(self, user_id: str, connection_id: str) -> None:
        ...
        self.stats["active_sessions"] = sum(
            len(connections) for connections in self.active_subscribers.values()
        )
        logger.info(
            f"User {user_id} subscribed to progress updates (connection: {connection_id})"
        )
    ...
    async def unsubscribe_user(self, user_id: str, connection_id: str) -> None:
        ...
        logger.info(
            f"User {user_id} unsubscribed from progress updates (connection: {connection_id})"
        )
    ...
    async def subscribe_instructor(self, connection_id: str) -> None:
        ...
        logger.info(
            f"Instructor subscribed to all progress updates (connection: {connection_id})"
        )
    ...
    async def unsubscribe_instructor(self, connection_id: str) -> None:
        ...
        logger.info(
            f"Instructor unsubscribed from all progress updates (connection: {connection_id})"
        )
    ...
    def get_assignment_progress(
        self, user_id: str, assignment_id: str
    ) -> Optional[AssignmentProgressInfo]:
        """課題進捗を取得する"""
