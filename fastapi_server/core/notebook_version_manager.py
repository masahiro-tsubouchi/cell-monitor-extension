...
class NotebookVersionManager:
    ...
    def __init__(self):
        # バージョン履歴ストレージ
        self.version_histories: Dict[str, VersionHistory] = {}  # notebook_path -> VersionHistory
        self.versions: Dict[str, NotebookVersion] = {}  # version_id -> NotebookVersion
        self.snapshots: Dict[str, NotebookSnapshot] = {}  # snapshot_id -> NotebookSnapshot
        self.branches: Dict[str, NotebookBranch] = {}  # branch_id -> NotebookBranch
        self.tags: Dict[str, VersionTag] = {}  # tag_id -> VersionTag
        
        # 分析データ
        self.analytics: Dict[str, VersionAnalytics] = {}  # notebook_path -> VersionAnalytics
    ...
    async def commit_version(
        ...
        try:
            ...
            # 現在のブランチを取得
            current_branch = self._get_or_create_branch(notebook_path, branch_name, author_id)
            ...
            logger.info(f"Committed version {version_number} ({version_id}) for {notebook_path}")
            return version
    ...
    async def create_branch(
        ...
        try:
            ...
            logger.info(f"Created branch {branch_name} ({branch_id}) for {notebook_path}")
            return branch
            
        except Exception as e:
            logger.error(f"Failed to create branch {branch_name} for {notebook_path}: {e}")
            raise
    ...
    async def compare_versions(
        ...
        try:
            ...
            # 差分を計算
            diffs = await self._calculate_diffs(from_version.snapshot, to_version.snapshot)
            
            # 変更統計を計算
            summary = {
                'total_changes': len(diffs),
                'cells_added': sum(1 for d in diffs if d.change_type == ChangeType.ADDED),
                'cells_deleted': sum(1 for d in diffs if d.change_type == ChangeType.DELETED),
                'cells_modified': sum(1 for d in diffs if d.change_type == ChangeType.MODIFIED),
                'cells_moved': sum(1 for d in diffs if d.change_type == ChangeType.MOVED),
            ...
            # 類似度スコアを計算
            similarity_score = self._calculate_similarity_score(from_version.snapshot, to_version.snapshot)
            
            # 変更の重要度を判定
            change_significance = self._assess_change_significance(summary, similarity_score)
            
            # 推奨アクションを生成
            recommended_actions = self._generate_comparison_recommendations(summary, similarity_score)
            ...
            logger.info(f"Compared versions {from_version.version_number} -> {to_version.version_number}")
            return comparison
    ...
    async def _create_cell_diff(
        ...
        if change_type == ChangeType.MODIFIED and old_content and new_content:
            ...
            diff.lines_modified = min(len(old_lines), len(new_lines)) - max(0, len(old_lines) + len(new_lines) - diff.lines_added - diff.lines_deleted)
    ...
    async def _detect_cell_moves(
        ...
        # 既存の差分から移動候補を特定
        for diff in diffs:
            if (diff.change_type == ChangeType.MODIFIED and 
                diff.cell_id in from_indices and 
                diff.cell_id in to_indices):
                ...
                if old_index != new_index:
                    ...
                    # 内容が同じ場合は移動のみ
                    if (diff.old_content and diff.new_content and 
                        diff.old_content.source == diff.new_content.source):
    ...
    def _calculate_similarity_score(
        ...
        try:
            # 基本統計の類似度
            total_cells_similarity = 1.0 - abs(from_snapshot.total_cells - to_snapshot.total_cells) / max(from_snapshot.total_cells, to_snapshot.total_cells, 1)
            total_lines_similarity = 1.0 - abs(from_snapshot.total_lines - to_snapshot.total_lines) / max(from_snapshot.total_lines, to_snapshot.total_lines, 1)
            ...
            if not from_text and not to_text:
                ...
                if max_len == 0:
                    ...
                    # 簡易的な差分計算
                    diff_ratio = difflib.SequenceMatcher(None, from_text, to_text).ratio()
    ...
    def _assess_change_significance(self, summary: Dict[str, int], similarity_score: float) -> str:
        """変更の重要度を評価する"""
        ...
        # 大きな変更の判定
        if (total_changes > 10 or 
            lines_added > 100 or 
            lines_deleted > 100 or 
            similarity_score < 0.3):
            return "breaking"
        
        # 中程度の変更の判定
        if (total_changes > 3 or 
            lines_added > 20 or 
            lines_deleted > 20 or 
    ...
    def _generate_comparison_recommendations(
        ...
        if lines_added > lines_deleted * 2:
            recommendations.append("Significant code expansion detected - consider reviewing for complexity")
        
        if lines_deleted > lines_added * 2:
            recommendations.append("Significant code reduction detected - verify functionality is preserved")
        
        if similarity_score < 0.5:
            recommendations.append("Major structural changes detected - thorough testing recommended")
        
        if total_changes > 20:
            recommendations.append("Many changes detected - consider breaking into smaller commits")
    ...
    def _generate_version_number(self, history: VersionHistory, branch_name: str) -> str:
        """バージョン番号を生成する"""
        
        # ブランチ別のバージョン数を計算
        branch_versions = [
            v for v in history.versions 
            if v.version_number.startswith(f"{branch_name}-") or 
               (branch_name == "main" and not "-" in v.version_number)
        ]
    ...
    async def _initialize_version_history(self, notebook_path: str, author_id: str) -> None:
        """バージョン履歴を初期化する"""
    ...
    def get_latest_version(self, notebook_path: str, branch_name: str = "main") -> Optional[NotebookVersion]:
        """最新バージョンを取得する"""
