...
class EnvironmentCollector:
    ...
    async def collect_environment_snapshot(
        ...
        try:
            ...
            is_full_snapshot = (
                ...
                or self._should_do_full_collection()
            )
    ...
    async def _collect_python_environment(
        self, is_full_snapshot: bool
    ) -> PythonEnvironmentInfo:
        """Python環境情報を収集する"""
    ...
    async def _collect_jupyter_environment(self) -> JupyterEnvironmentInfo:
        ...
        try:
            ...
            # セキュリティ向上のため完全パス指定とshell=Falseを明示
            import shlex
            import shutil
            ...
            if jupyter_path:
                result = subprocess.run(
                    [jupyter_path, "labextension", "list", "--json"],
                    capture_output=True,
                    text=True,
                    timeout=10,
                    shell=False,  # セキュリティのため明示的にFalse
                )
            else:
                result = None
    ...
    async def _collect_package_info(self) -> Tuple[List[PackageInfo], int]:
        ...
        if pkg_resources:
            try:
                # インストール済みパッケージを取得
                installed_packages = {
                    pkg.project_name.lower(): pkg for pkg in pkg_resources.working_set
                }
    ...
    def _detect_package_changes(
        self, current_python_env: PythonEnvironmentInfo
    ) -> List[str]:
        """パッケージの変更を検出する"""
        changed_packages = []
        ...
        # 現在のパッケージ情報をマップに変換
        current_packages = {
            pkg.name.lower(): pkg.version for pkg in current_python_env.key_packages
        }
        ...
        for pkg_name, version in current_packages.items():
            if pkg_name not in last_packages:
                ...
                changed_packages.append(
                    f"{pkg_name} (updated: {last_packages[pkg_name]} -> {version})"
                )
    ...
    def create_environment_diff(
        ...
        # パッケージ変更を分析
        from_packages = {
            pkg.name.lower(): pkg for pkg in from_snapshot.python_env.key_packages
        }
        to_packages = {
            pkg.name.lower(): pkg for pkg in to_snapshot.python_env.key_packages
        }
        ...
        for pkg_name, pkg_info in to_packages.items():
            if (
                pkg_name in from_packages
                and from_packages[pkg_name].version != pkg_info.version
            ):
...
async def collect_current_environment(
    ...
    return await environment_collector.collect_environment_snapshot(
        notebook_path=notebook_path, cell_id=cell_id, execution_count=execution_count
    )
