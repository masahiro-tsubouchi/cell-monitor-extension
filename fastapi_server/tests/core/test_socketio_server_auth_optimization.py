...
class TestSocketIOServerAuthOptimization:
    ...
    @pytest.mark.asyncio
    async def test_auth_performance_optimization(
        ...
        with patch("core.socketio_server.get_db") as mock_get_db, patch(
            ...
            # 検証
            assert (
                successful_auths == concurrent_requests
            ), f"Expected {concurrent_requests} successful auths, got {successful_auths}"
            assert (
                average_auth_time < 0.1
            ), f"Average auth time too slow: {average_auth_time:.3f}s"
    ...
    @pytest.mark.asyncio
    async def test_auth_logging_optimization(
        self, socketio_manager, mock_instructor, valid_token
    ):
        """認証ログ最適化テスト"""
        with patch("core.socketio_server.get_db") as mock_get_db, patch(
            ...
            for scenario in test_scenarios:
                ...
                if scenario["name"] == "invalid_token":
                    ...
                    mock_verify_token.return_value = (
                        {"sub": mock_instructor.email} if scenario["auth"] else None
                    )
                    ...
                    if not token:
                        ...
                        try:
                            ...
                            if payload:
                                # 成功ログをシミュレート
                                mock_logger.info(
                                    f"Socket.IO instructor connected successfully (sid: {sid})"
                                )
                                mock_logger.debug(
                                    f"Token verification successful (sid: {sid})"
                                )
                                ...
                                mock_logger.error(
                                    f"Socket.IO connection rejected: Token verification failed (sid: {sid})"
                                )
                                await socketio_manager.sio.disconnect(sid)
                                result = False
                        except Exception as e:
                            mock_logger.error(
                                f"Socket.IO connection rejected: Token verification failed (sid: {sid}), error: {str(e)}"
                            )
                            await socketio_manager.sio.disconnect(sid)
                            result = False
                except Exception:
                ...
                for log_level in scenario["expected_logs"]:
                    ...
                    assert (
                        log_method.called
                    ), f"Expected {log_level} log not called for scenario {scenario['name']}"

    @pytest.mark.asyncio
    async def test_auth_error_response_optimization(
        ...
        with patch("core.socketio_server.get_db") as mock_get_db, patch(
            ...
            for scenario in error_scenarios:
                ...
                if scenario["expected_error_event"]:
                    ...
                    assert (
                        error_event_sent
                    ), f"Expected error event {scenario['expected_error_event']} not sent for scenario {scenario['name']}"

    @pytest.mark.asyncio
    async def test_auth_database_connection_optimization(
        self, socketio_manager, mock_instructor
    ):
        """
        データベース接続の最適化をテスト
        ...
        with patch("core.socketio_server.get_db") as mock_get_db, patch(
            ...
            # データベース接続最適化の検証
            assert (
                len(db_connections) == auth_requests
            ), f"Expected {auth_requests} DB connections, got {len(db_connections)}"

            # 各接続が適切にクローズされたことを確認
            for db in db_connections:
                assert hasattr(
                    db, "close"
                ), "Database connection should have close method"
    ...
    @pytest.mark.asyncio
    async def test_auth_memory_usage_optimization(
        self, socketio_manager, mock_instructor, valid_token
    ):
        """認証メモリ使用量最適化テスト"""
        with patch("core.socketio_server.get_db") as mock_get_db, patch(
