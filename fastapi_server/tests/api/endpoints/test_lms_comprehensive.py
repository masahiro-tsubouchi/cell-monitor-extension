...
class TestLMSIntegrationWorkflow:
    ...
    def test_complete_lms_workflow(self):
        ...
        # 4. 提出確認・評価
        # 提出リストの確認
        submissions_list_response = client.get(f"/api/v1/submissions/?assignment_id={assignment_id}")
        ...
        evaluation_response = client.put(f"/api/v1/submissions/{submission_id}", json=evaluation_data)
    ...
    def test_multi_student_assignment_workflow(self):
        ...
        # 全提出の確認
        all_submissions_response = client.get(f"/api/v1/submissions/?assignment_id={assignment_id}")
    ...
    def test_assignment_deadline_workflow(self):
        ...
        # クラス内の全課題確認
        class_assignments_response = client.get(f"/api/v1/assignments/?class_id={class_id}")
...
class TestLMSDataIntegrity:
    ...
    def test_assignment_submission_relationship(self):
        ...
        # 課題に対する全提出確認
        assignment_submissions_response = client.get(f"/api/v1/submissions/?assignment_id={assignment_id}")
    ...
    def test_student_submission_history(self):
        ...
        # 学生の全提出履歴確認
        student_submissions_response = client.get(f"/api/v1/submissions/?student_id={student_id}")
        ...
        current_test_submissions = [s for s in student_submissions if s["assignment_id"] in assignment_ids]
        assert len(current_test_submissions) >= 2  # 現在のテストで作成した2件の提出が存在することを確認
...
class TestLMSErrorHandling:
    ...
    def test_invalid_class_assignment_creation(self):
        ...
        assignment_response = client.post("/api/v1/assignments/", json=invalid_assignment_data)
    ...
    def test_duplicate_submission_handling(self):
        ...
        first_submission_response = client.post("/api/v1/submissions/", json=submission_data)
        assert first_submission_response.status_code == 201
        first_submission_id = first_submission_response.json()["id"]
        
        # 新規作成ではなく既存提出の更新を使用
        update_response = client.put(f"/api/v1/submissions/{first_submission_id}", json={
            "status": "resubmitted"
        })
