...
class TestNotebookVersionAPI:
    ...
    def test_commit_notebook_version_success(self, mock_commit_version, mock_get_snapshot):
        """バージョンコミット成功テスト"""
    ...
    @patch('api.endpoints.notebook_version.notebook_version_manager.get_snapshot')
    @patch('api.endpoints.notebook_version.notebook_version_manager.commit_version')
    def test_commit_notebook_version_error(self, mock_commit_version, mock_get_snapshot):
        """バージョンコミットエラーテスト"""
        # モックスナップショット（get_snapshot用）
        mock_snapshot = NotebookSnapshot(
            snapshot_id="snap_123",
...
class TestNotebookHistoryAPI:
    ...
    @patch('api.endpoints.notebook_version.notebook_version_manager.get_version_history')
    ...
    def test_get_latest_version_success(self, mock_get_latest):
        ...
        response = client.get("/api/v1/v1/notebook-version/latest?notebook_path=test_notebook.ipynb&branch_name=main")
...
class TestNotebookVersionIntegration:
    ...
    @patch('api.endpoints.notebook_version.notebook_version_manager.commit_version')
    @patch('api.endpoints.notebook_version.notebook_version_manager.get_version_history')
    def test_full_version_workflow(self, mock_get_history, mock_commit_version, mock_create_snapshot, mock_get_snapshot):
        """完全バージョン管理ワークフローテスト"""
        # スナップショット作成
        mock_snapshot = NotebookSnapshot(
        ...
        # 3. 履歴確認
        history_response = client.get("/api/v1/v1/notebook-version/history/workflow_notebook.ipynb")
