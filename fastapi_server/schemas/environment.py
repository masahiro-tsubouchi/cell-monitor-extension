...
class JupyterEnvironmentInfo(BaseModel):
    ...
    jupyter_core_version: Optional[str] = Field(
        None, description="Jupyter Coreバージョン"
    )
    ...
    extensions: List[Dict[str, str]] = Field(
        default_factory=list, description="インストール済み拡張機能リスト"
    )
...
class ExecutionEnvironmentSnapshot(BaseModel):
    ...
    changed_packages: List[str] = Field(
        default_factory=list, description="前回から変更されたパッケージ名"
    )

    # メタデータ
    collection_duration_ms: Optional[float] = Field(
        None, description="環境情報収集にかかった時間(ms)"
    )
...
class EnvironmentDiff(BaseModel):
    ...
    added_packages: List[PackageInfo] = Field(
        default_factory=list, description="追加されたパッケージ"
    )
    removed_packages: List[str] = Field(
        default_factory=list, description="削除されたパッケージ名"
    )
    ...
    system_changes: Dict[str, Any] = Field(
        default_factory=dict, description="システム環境の変更"
    )

    # Jupyter環境変更
    jupyter_changes: Dict[str, Any] = Field(
        default_factory=dict, description="Jupyter環境の変更"
    )
...
class EnvironmentAnalytics(BaseModel):
    ...
    environment_health_score: float = Field(
        ..., ge=0.0, le=1.0, description="環境健全性スコア (0.0-1.0)"
    )

    # 推奨事項
    recommendations: List[str] = Field(
        default_factory=list, description="環境改善の推奨事項"
    )

    # 警告・注意事項
    warnings: List[str] = Field(default_factory=list, description="環境に関する警告")

    # パフォーマンス指標
    performance_indicators: Dict[str, Any] = Field(
        default_factory=dict, description="パフォーマンス関連指標"
    )

    # 互換性情報
    compatibility_info: Dict[str, Any] = Field(
        default_factory=dict, description="パッケージ間の互換性情報"
    )
