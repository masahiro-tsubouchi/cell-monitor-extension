...
class NotebookSnapshot(BaseModel):
    ...
    def __init__(self, **data):
        ...
        if self.cells:
            ...
            self.code_cells_count = sum(1 for cell in self.cells if cell.cell_type == CellType.CODE)
            self.markdown_cells_count = sum(1 for cell in self.cells if cell.cell_type == CellType.MARKDOWN)
...
class NotebookVersion(BaseModel):
    ...
    status: VersionStatus = Field(VersionStatus.DRAFT, description="バージョンステータス")
    ...
    def __init__(self, **data):
        ...
        if self.diffs:
            ...
            self.cells_added = sum(1 for diff in self.diffs if diff.change_type == ChangeType.ADDED)
            self.cells_deleted = sum(1 for diff in self.diffs if diff.change_type == ChangeType.DELETED)
            self.cells_modified = sum(1 for diff in self.diffs if diff.change_type == ChangeType.MODIFIED)
            self.cells_moved = sum(1 for diff in self.diffs if diff.change_type == ChangeType.MOVED)
...
class NotebookBranch(BaseModel):
    ...
    branched_from_version_id: Optional[str] = Field(None, description="分岐元バージョンID")
...
class VersionHistory(BaseModel):
    ...
    # バージョン一覧
    versions: List[NotebookVersion] = Field(default_factory=list, description="バージョン一覧")
    
    # ブランチ一覧
    branches: List[NotebookBranch] = Field(default_factory=list, description="ブランチ一覧")
    ...
    def __init__(self, **data):
        ...
        if self.versions:
            ...
            self.total_commits = len([v for v in self.versions if v.status == VersionStatus.COMMITTED])
...
class VersionComparison(BaseModel):
    ...
    change_significance: str = Field(
        "minor", 
        description="変更の重要度 (minor/major/breaking)"
    )
...
class VersionTag(BaseModel):
    ...
    # タグタイプ
    tag_type: str = Field("release", description="タグタイプ (release/milestone/checkpoint)")
