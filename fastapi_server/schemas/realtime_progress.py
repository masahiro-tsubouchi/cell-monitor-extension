...
class CellProgressInfo(BaseModel):
    ...
    execution_status: CellExecutionStatus = Field(
        CellExecutionStatus.PENDING, description="実行ステータス"
    )
    ...
    completion_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="完了度スコア (0.0-1.0)"
    )
...
class AssignmentProgressInfo(BaseModel):
    ...
    overall_status: ProgressStatus = Field(
        ProgressStatus.NOT_STARTED, description="全体ステータス"
    )
    ...
    cells_progress: List[CellProgressInfo] = Field(
        default_factory=list, description="セル別進捗詳細"
    )
    ...
    difficulty_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="難易度スコア (0.0-1.0)"
    )
    engagement_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="取り組み度スコア (0.0-1.0)"
    )
...
class StudentProgressSummary(BaseModel):
    ...
    overall_progress_percentage: float = Field(
        0.0, ge=0.0, le=100.0, description="全体進捗率 (%)"
    )

    # 課題別進捗
    assignments_progress: List[AssignmentProgressInfo] = Field(
        default_factory=list, description="課題別進捗詳細"
    )
    ...
    error_rate: float = Field(0.0, ge=0.0, le=1.0, description="エラー率 (0.0-1.0)")
    ...
    learning_velocity: Optional[float] = Field(
        None, description="学習速度 (課題完了数/時間)"
    )
    focus_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="集中度スコア (0.0-1.0)"
    )
    consistency_score: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="継続性スコア (0.0-1.0)"
    )
...
class ProgressNotification(BaseModel):
    ...
    level: NotificationLevel = Field(NotificationLevel.INFO, description="通知レベル")
...
class ProgressAnalytics(BaseModel):
    ...
    performance_metrics: Dict[str, float] = Field(
        default_factory=dict, description="パフォーマンス指標"
    )

    # 学習パターン
    learning_patterns: Dict[str, Any] = Field(
        default_factory=dict, description="学習パターン分析結果"
    )

    # 予測情報
    predictions: Dict[str, Any] = Field(
        default_factory=dict, description="完了時間・成績予測等"
    )

    # 推奨事項
    recommendations: List[str] = Field(
        default_factory=list, description="学習改善の推奨事項"
    )

    # 比較情報
    peer_comparison: Optional[Dict[str, Any]] = Field(
        None, description="同級生との比較情報"
    )

    # 信頼度
    confidence_score: float = Field(
        0.0, ge=0.0, le=1.0, description="分析結果の信頼度 (0.0-1.0)"
    )
...
class RealtimeProgressConfig(BaseModel):
    ...
    # 更新頻度設定
    update_interval_seconds: int = Field(5, ge=1, le=300, description="更新間隔(秒)")
